...
func runPostTestDestroy(t testing.T, c TestCase, wd *plugintest.WorkingDir, factories map[string]func() (*schema.Provider, error), v5factories map[string]func() (tfprotov6.ProviderServer, error), statePreDestroy *terraform.State) error {
	t.Helper()
...
func runNewTest(t testing.T, c TestCase, helper *plugintest.Helper) {
	...
	defer func() {
		...
		err = runProviderCommand(t, func() error {
			...
			return nil
		}, wd, c.ProviderFactories, c.ProtoV6ProviderFactories)
		if err != nil {
		...
		if !stateIsEmpty(statePreDestroy) {
			err := runPostTestDestroy(t, c, wd, c.ProviderFactories, c.ProtoV6ProviderFactories, statePreDestroy)
			if err != nil {
